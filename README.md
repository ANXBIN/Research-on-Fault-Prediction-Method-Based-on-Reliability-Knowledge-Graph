...
│   └── gnn_kg_v2_model.pt   # GNN-KG-V2 (KG投影增强)
...
python train.py --gnn-kg-v2       # GNN-KG-V2 (KG投影增强)
```
...
python predict.py --model GNN-KG-V2 --sample
...
| MLP (基准) | 66.8% | 0.659 |
| MLP-KG | 75.8% | 0.756 |
| MLP-KG-V2 | 76.2% | 0.763 |
| CNN | 70.8% | 0.699 |
| CNN-KG | 78.9% | 0.775 |
| CNN-KG V2 | 80.1% | 0.795 |
| CNN-KG V3 | 80.1% | 0.796 |
| GNN | 76.8% | 0.767 |
| **GNN-KG** | **83.6%** | **0.835** |
| **GNN-KG-V2** | **83.6%** | **0.835** |

### 最佳模型：GNN-KG / GNN-KG-V2

GNN-KG和GNN-KG-V2并列最高准确率 **83.6%**。GNN-KG-V2在V1基础上增加了KG嵌入投影增强层（33→128维）和深层融合残差块，但性能与V1基本持平，说明V1的早期拼接融合架构已经充分利用了33维故障级别KG嵌入中的信息。
...
|------|----------|------|
| MLP | 无 | 普通多层感知机（基准） |
| MLP-KG | KNN样本嵌入(64维) | 基于K近邻的样本级嵌入，拼接融合 |
| MLP-KG-V2 | KNN样本嵌入(64维) | 门控融合架构，自适应权重调整 |
...
```
输入特征 ──→ 特征编码器 ──┐
                          ├── Sigmoid门控 ──→ gated = gate*feat + (1-gate)*kg
KNN嵌入  ──→ KG编码器  ──┘                        ↓
                              concat(feat, kg, gated) ──→ 深层融合 ──→ 分类
```
...
| GNN-KG-V2 | GNN-KG + KG投影增强 + 深层融合残差 |
...
**GNN-KG-V2改进**：
- KG投影增强：33维KG嵌入先投影到128维，再与特征拼接
- 深层融合残差块：GCN输出后增加额外残差连接，增强梯度流动
