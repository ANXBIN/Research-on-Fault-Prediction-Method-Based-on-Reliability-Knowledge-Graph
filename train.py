...
class Trainer:
    ...
    def load_data(self):
        ...
        print(f"KG嵌入维度: {self.kg_train_emb.shape}")
    ...
    def train_kg_mlp_v1(self):
        """训练KG增强MLP V1"""
        print("\n" + "=" * 60)
        print("训练 KG-Enhanced MLP 模型 (知识图谱增强 V1)")
        print("=" * 60)
...
def main():
    ...
    parser.add_argument('--kg-v1', action='store_true', help='训练KG-MLP V1模型')
    parser.add_argument('--kg-v2', action='store_true', help='训练KG-MLP V2模型')
    parser.add_argument('--all', action='store_true', help='训练所有模型')
    args = parser.parse_args()

    # 默认训练所有模型
    train_mlp = args.all or args.mlp or (not (args.kg_v1 or args.kg_v2))
    train_kg_v1 = args.all or args.kg_v1 or (not (args.mlp or args.kg_v2))
    train_kg_v2 = args.all or args.kg_v2 or (not (args.mlp or args.kg_v1))

    if not (train_mlp or train_kg_v1 or train_kg_v2):
        print("请选择要训练的模型，使用 --mlp, --kg-v1, --kg-v2 或 --all")
        return
